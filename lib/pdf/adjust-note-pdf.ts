...
export async function generateAdjustNotePDF(
  ...
  doc.text("NOTA DE AJUSTE", pageW / 2, y + 10, { align: "center" })
  ...
  const rows: { label: string; value: string }[] = [
    ...
    { label: "ALMACÉN", value: movement.fromWarehouseName ?? movement.toWarehouseName ?? "—" },
    { label: "PRODUCTO", value: movement.productName },
    { label: "SKU", value: movement.productSku },
    { label: "STOCK ANTERIOR", value: movement.stockBefore != null ? String(movement.stockBefore) : "—" },
    { label: "STOCK NUEVO", value: movement.stockAfter != null ? String(movement.stockAfter) : "—" },
    {
      label: "DIFERENCIA",
      value: `${movement.quantity} ${movement.quantity != null && movement.quantity >= 0 ? "(+)" : "(-)"}`,
    },
